...
class Translator:
    ...
    def parse_label(self, line):
        ...
        if self.current_section == Section.CODE:
            assert not (label in self.code_labels)
            if label == "_start":
                self.start = hex(self.code_address)
            self.code_labels[label] = self.code_address
        else:
            assert not (label in self.data_labels)
        ...
        for line in lines:
            ...
            match type:
                ...
                case LineType.UNKNOWN:
                    raise Exception("Internal error: unknown code line")
    ...
    def process_addressing(self, command, type: Opcode):
        ...
        result = result // 2**8
        bytes[1] += result
        return struct.pack("BBBB", bytes[0], bytes[1], bytes[2], bytes[3])

    def process_non_addressing(self, command, type: Opcode):
        args = command["args"]
        bytes = [type.value, 0, 0, 0]
        arguments = [0, 0, 0]
    ...
    def translate_stage_3(self):
        ...
        for command in self.code:
            ...
            type: Opcode = instructions[command["opcode"]]["type"]
            if addressing:
                result = self.process_addressing(command, type)
            else:
                result = self.process_non_addressing(command, type)
